...
def plot_data(hyps, algebra, seed: int = 1233, nruns: int = 100, y_axis = True):
    ...
    for i, n, in enumerate(nqubit_list):
        axs.plot(depth_list, grads_var[i, :], label=rf'$n = $ {n}')
    axs.set_xlabel('depth')
    axs.set_yscale('log')
    axs.set_ylim([1e-6, 1e-1])
    axs.legend(prop={'size': 8})
    if algebra=='a14':
    	axs.set_title(r'(b) $\mathfrak{a}_{14}(n)$', y=-0.25)
    if algebra=='a5_circ':
    	axs.set_title(r'(a) $\mathfrak{a}_{5}^\circ(n)$', y=-0.25)

    plt.tight_layout()

    fig.savefig(fig_path + f'{algebra}_scaling_variance.pdf')
    fig.savefig(fig_path + f'{algebra}_scaling_variance.png',dpi=300)
...
def plot_data_combined(hyps, algebra_1, algebra_2, seed: int = 1233, nruns: int = 100):
    ...
    fig.savefig(fig_path + f'{algebra_1}_{algebra_2}_scaling_variance.png',dpi=300)
